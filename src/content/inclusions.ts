/**
 * The client inclusion schedule, migrated verbatim from the live site's
 * `src/lib/inclusions.ts`. Item names, allowances, upgrade options, notes,
 * prices and exclusions are unchanged.
 *
 * The only edit is `inclusionStatusMeta.className`: the live site used raw
 * hex utilities, which this codebase does not allow in a component. The
 * treatments below are the same four states expressed in theme tokens.
 */
...
export const buildTypeAdjustmentMap: Record<
  BuildType,
  { label: string; amount: number }
> = {
  "single-storey": { label: "Single Storey", amount: 0 },
  "double-storey": { label: "Double Storey", amount: 152000 },
  custom: { label: "Complete Custom", amount: 0 },
};
...
> = {
  included: {
    label: "Included",
    className: "border border-brand bg-brand text-white",
  },
  allowance: {
    label: "Allowance",
    className: "border border-line-strong bg-surface text-brand",
  },
  upgrade: {
    label: "Upgrade",
    className: "border border-brand bg-mist text-brand",
  },
  excluded: {
    label: "Excluded",
    className: "border border-line bg-white text-muted",
  },
};
...
export function buildInclusionSummary(
  ...
  const selectedPackage =
    inclusionPackages.find((item) => item.id === packageId) ||
    inclusionPackages[1];
  ...
  return {
    ...
    upgradesTotal,
    estimatedTotal:
      selectedPackage.basePrice + buildTypeAdjustment + upgradesTotal,
  };
}
